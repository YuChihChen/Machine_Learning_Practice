...
"""
# ====== get data ======
df_in = pd.read_csv('data/College_ISLRinR.csv', header=0)
df_in.drop(columns='Unnamed: 0', inplace=True)
df_in['Private_fac'] = pd.factorize(df_in['Private'])[0]
df_in.drop(columns='Private', inplace=True)
df_in.columns = df_in.columns.str.replace('.','_')
print(df_in.head())

# ====== scale df and split train_test_dfs for CV ======
cv = acv.CrossValidation(df_in)
cv.scale_predictors('Apps')
...
error_ols, error_std_ols = cv.cv_get_test_errors([model_ols], 'Apps')
...
errors_ridge, errors_std_ridges = cv.cv_get_test_errors(models_ridge, 'Apps', plot_=True,lambdas_=lambdas)
...
errors_lasso, errors_std_lasso = cv.cv_get_test_errors(models_lasso, 'Apps', plot_=True,lambdas_=lambdas)
...
errors_pls, errors_std_pls = cv.cv_get_test_errors(models_pls, 'Apps', plot_=True,lambdas_=m_dims)
